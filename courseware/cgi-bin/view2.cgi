...
<table border="0" width="90%" cellpadding="2" cellspacing="0" bgcolor="#E2E2E2"> <tr> <td> <font size="2" face="Arial, Helvetica, sans-serif"><b><font color="#999999" size="3">@toast[2]</font></b></font> </td> </tr> </table>
EndBigLine
	    if (-f "$path/$program") {  # if the program exists
		#print "<hr noshade>";
		#print $program;

		#print '<font size=-1 face="verdana, sans-serif"><b>';
		#print "@toast[2]";
		#print '</font></b>';
		print "<br> <br>";
		&showstuff("$path/$program");
		print "<br><br><br>";
...
END_of_midtable2
	...
	$temp0 =~ s/[0-9]+//;
	#print("temp0 = $temp0\n");
	print("<b>Problem $prob_num</b> -- $problems[$prob_num - 1][$let_to_num{$temp0}]<br>");
    } else {
	# this is where the information regarding the current problem SET is written
	$temp0 = $prob_num;
	$temp0 =~ s/[A-Z]//;
	print("<font face=\"Arial, Helvetica, sans-serif\" size=\"2\" color=\"#666666\">");
	print("<b>Problem Set $temp0</b> -- $headers[$temp0 - 1]<br>");
    }
print("<br>");

# modified to go to view2.cgi instead of view.cgi
print<<END_of_midtable25;				
	<hr size="1" width="95%" noshade align="left">
      <form method="post" action="view2.cgi">   
...
for $i (0 .. $#headers) {
	...
	if ($prob_num eq "$count$let[$j]") {
	    print("<option value=\"$count$let[$j]\" selected>Problem $count$let[$j]</option>\n");
	} else {
	    print("<option value=\"$count$let[$j]\">Problem $count$let[$j]</option>\n");
	}
    }
    $count++;
}
...
if ($person eq "*everyone*") {
    print("<option value=\"*everyone*\" selected>Everyone</option>\n");
} else {
    print("<option value=\"*everyone*\">Everyone</option>\n");
}
...
print<<END_of_midtable4;
	    ...
	    <input type=hidden name="course" value="$course">
            <td valign="top"> &nbsp;&nbsp;<input type="submit" name="submit" value="Do it"></td>
          </tr>
	</table>
      </form>
...
sub showstuff {
	...
	if ($line =~ /^\/\//) { # if line *begins with* a comment
	    $line =~ s/(\/\/.*)$/<font color=\"\#999999\">$1 <\/font>/;
	}

	if ($line =~ /http\:\/\// ) {
	    $line =~ s/(http\:\/\/\S*)/<a target=\"$what\" href=\"$1\">$1<\/a>/g;
	} elsif ($line =~ /www\./) {
	    # the smartass didn't include http
	    $line =~ s/(www\.\S*)/<a target=\"$what\" href=\"http\:\/\/$1\">$1<\/a>/g;
	} 
	print "$line\r\n";
    }
    print "</PRE>";
