...
public class GifEncoder
{
  ...
  */
  ...
  public GifEncoder(int pixels[], int width, OutputStream out,
		    boolean interlace) throws IOException {
    this.imageData = pixels;
    this.imageWidth = width;
    this.imageHeight = pixels.length / width;
  ...
  public void encode() throws IOException {
	...
	int rgb = imageData[row * imageWidth + col];
	...
	if (isTransparent) {
	  if (transparentIndex < 0) {
	    ...
	    // the first one.
	    imageData[row * imageWidth + col] = rgb = transparentRgb;
	  }
	}
    ...
    gifEncode(out, imageWidth, imageHeight, interlace, 
  ...
  byte GetPixel(int x, int y) throws IOException {
    GifEncoderHashitem item =
      (GifEncoderHashitem) colorHash.get(imageData[y * imageWidth + x]);
    if (item == null)
  ...
  // Rowley <mgardi@watdscu.waterloo.edu>.  Lempel-Ziv compression
  // based on "compress".
