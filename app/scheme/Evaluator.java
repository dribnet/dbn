...
{   
	...
	// System.out.println(ListNode.length(env)+ " eval: "+L);

	try {
	    while(true) {
		if (kill) {
		    //ThrowError.error("Killing Evaluator while evaluating"+L+"\n");
		    //return null;
		    //return "( )";
		    //stop();
		    //return null;
		    //throw new DbnException("not really");
		    throw new RuntimeException("not really");
		}
		
		if (rt.freeMemory() < 10000) {
		    rt.gc();
		    if (rt.freeMemory()<20000) {
			ThrowError.error("Running out of memory!");
			System.out.print("            freemem = "+rt.freeMemory()+
					 " totalmem= "+rt.totalMemory()+"\n");
		    }
		    if (rt.freeMemory()<1000) {
			ThrowError.error("Quitting..... No more memory!");
			return null;
		    }
		}
	...
	if (old_debugging){
	    depth--;
	    System.out.println(" <<"+ depth+ ": "+ F.getName()+" applied to "+ argList +" returns with "+ res);
	}
	return res;
    }
}
